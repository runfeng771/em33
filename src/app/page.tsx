...
export default function Home() {
  ...
  const fetchEmails = async (forceRefresh = false) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (forceRefresh) {
          // Force refresh - replace all emails
          setEmails(data)
        } else {
          // Intelligent merge - only add new emails that don't exist
          const existingEmailIds = new Set(emails.map(e => e.id))
          const newEmails = data.filter((email: Email) => !existingEmailIds.has(email.id))
          
          if (newEmails.length > 0) {
            // Add new emails at the beginning of the list
            setEmails(prevEmails => [...newEmails, ...prevEmails])
            console.log(`Added ${newEmails.length} new emails to existing list`)
          } else {
  ...
  const refreshEmails = async (accountId?: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // 如果刷新的是当前选中的账号，智能获取邮件（不重新渲染已存在的邮件）
        if (targetAccountId === selectedAccount) {
          await fetchEmails(false) // Use intelligent merging
        }
  ...
  const forceRefreshEmails = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // 如果当前选中的账号在刷新结果中，重新获取该账号的邮件（强制刷新）
        if (selectedAccount) {
  ...
  useEffect(() => {
    if (selectedAccount) {
      fetchEmails(true) // Force refresh when filters change
    }
                        ...
                        <div>
                          ...
                          <div className="prose prose-sm max-w-none">
                            {selectedEmail.htmlBody ? (
                              <div 
                                className="text-sm text-gray-700 leading-relaxed bg-gray-50 p-6 rounded-xl border border-gray-200 shadow-sm max-h-96 overflow-y-auto custom-scrollbar"
                                dangerouslySetInnerHTML={{ __html: selectedEmail.htmlBody }}
                              />
                            ) : (
                              <div className="text-sm text-gray-700 leading-relaxed whitespace-pre-wrap bg-gray-50 p-6 rounded-xl border border-gray-200 shadow-sm max-h-96 overflow-y-auto custom-scrollbar">
                                {selectedEmail.body}
                              </div>
                            )}
                          </div>
